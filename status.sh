...
if command -v jq >/dev/null 2>&1; then
  echo ""
  echo "id           addr         health     power  mode  temp  fan   swing"
  echo "============ ===========  =========  =====  ====  ====  ====  ====="
  echo "$RESP" | jq -r '
    .[] |
    [
      (.id // ""),
      ((.ip // "") + ":" + ((.port // 0) | tostring)),
      (if (.health // {}) | .ok then "OK"
       else (if (.health // {}) | has("status_code") then "ERR " + ((.health.status_code // "") | tostring) else "N/A" end)
       end),
      ((.state // {}) | .power // ""),
      ((.state // {}) | .mode // ""),
      (((.state // {}) | .temp) // "" | tostring),
  ...
  ' | awk -F'\t' '{ printf "%-12s %-11s %-10s %-6s %-5s %-5s %-5s %-5s\n", $1,$2,$3,$4,$5,$6,$7,$8 }'
else
  echo ""
